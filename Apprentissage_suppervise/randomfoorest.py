...
from fonctions_plot import plot_learning_curve, correlation_train, correlation_test,plot_results
from sklearn.model_selection import learning_curve, validation_curve
import matplotlib.pyplot as plt
import numpy as np
import seaborn as sns
from scipy.stats import pearsonr
from fonctions_plot import plot_learning_curve,calculate_metrics_race,afficher_importance_features, correlation_train,afficher_importance_features, correlation_test,plot_results,plot_confusion,calculate_fairness_metrics, plot_permutation_importance
...
# Trouver l'indice de la colonne 'SEX'
...
cv_scores1 = cross_val_score(randomforest_model , X_train, Y_train, cv=5)
print(f"Score de cross validation : {cv_scores1.mean()}")


# Faire des prédictions sur les données de test
y_pred1 = randomforest_model.predict(X_test)
y_pred_train = randomforest_model.predict(X_train)

# Afficher les résultats
print(f"Accuracy Score pour random forest sur l'ensemble de test: {accuracy_score(Y_test, y_pred1)}")
print(f"Classification Report random forest sur l'ensemble de test:\n{classification_report(Y_test, y_pred1)}")
print("\n")

plot_learning_curve(randomforest_model , X_train , Y_train)

######GRid search 
param_grid_randomforest = {'n_estimators': [50, 100, 200]}#, 'max_depth': [None, 10, 20]}#, 'min_samples_split': [2, 5, 10]}

grid_search = GridSearchCV(randomforest_model, param_grid_randomforest, cv=5, scoring='accuracy')
grid_search.fit(X_train, Y_train)

# Afficher les meilleurs hyperparamètres pour chaque modèle
print(f"Meilleurs hyperparamètres pour {randomforest_model.__class__.__name__}: {grid_search.best_params_}")
...
print(f"Écart type des scores en validation croisée pour {randomforest_model.__class__.__name__}: {cv_scores_std}")

y_pred2 = grid_search.predict(X_test)

# Afficher les résultats
print(f"Accuracy Score pour {randomforest_model.__class__.__name__} sur l'ensemble de test: {accuracy_score(Y_test, y_pred2)}")
print(f"Classification Report pour {randomforest_model.__class__.__name__} sur l'ensemble de test:\n{classification_report(Y_test, y_pred2)}")
print("\n")

################# ANALYSE DU MODELE #################################################

afficher_importance_features(randomforest_model , X_all)

plot_permutation_importance(randomforest_model, X_test, Y_test, X_all.columns)

plot_confusion(Y_test, y_pred1)

plot_confusion(Y_test, y_pred2)

plot_results(randomforest_model, Y_test, y_pred1, y_pred2, cv_scores1, cv_scores2)
...
# Afficher les résultats
print(f"Accuracy Score pour {randomforest_model.__class__.__name__} sur l'ensemble de test nevada: {accuracy_score(y_test_ne, y_pred_ne)}")
print(f"Classification Report pour {randomforest_model.__class__.__name__} sur l'ensemble de test nevada :\n{classification_report(y_test_ne, y_pred_ne)}")
print("\n")
...
# Afficher les résultats
print(f"Accuracy Score pour {randomforest_model.__class__.__name__} sur l'ensemble de test colorado: {accuracy_score(y_test_co, y_pred_co)}")
print(f"Classification Report pour {randomforest_model.__class__.__name__} sur l'ensemble de test colorado :\n{classification_report(y_test_co, y_pred_co)}")
print("\n")
...
grid_search2 = GridSearchCV(randomforest_model2 , param_grid_randomforest, cv=5, scoring='accuracy')
