...
from fonctions_plot import plot_learning_curve, correlation_train,afficher_importance_features, correlation_test,calculate_metrics_race,plot_results,plot_confusion,calculate_fairness_metrics, plot_permutation_importance
...
cv_scores1 = cross_val_score(adaboost_model, X_train, Y_train, cv=5)
...
print(f"Accuracy Score pour adaboost sur l'ensemble de test: {accuracy_score(Y_test, y_pred1)}")
print(f"Classification Report pour adaboost sur l'ensemble de test:\n{classification_report(Y_test, y_pred1)}")
print(f"Confusion Matrix pour adaboost sur l'ensemble de test:\n{confusion_matrix(Y_test, y_pred1)}")
print("\n")

plot_learning_curve(adaboost_model, X_train, Y_train)

######GRid search 
param_grid_svm = {'n_estimators': [50, 100, 200], 'learning_rate': [0.01, 0.1, 1]}

grid_search = GridSearchCV(AdaBoostClassifier(), param_grid_svm, cv=5, scoring='accuracy')
grid_search.fit(X_train, Y_train)

# Afficher les meilleurs hyperparamètres pour chaque modèle
print(f"Meilleurs hyperparamètres pour {adaboost_model.__class__.__name__}: {grid_search.best_params_}")
...
print(f"Écart type des scores en validation croisée pour {adaboost_model.__class__.__name__}: {cv_scores_std}")

y_pred2 = grid_search.predict(X_test)

# Afficher les résultats
print(f"Accuracy Score pour {adaboost_model.__class__.__name__} sur l'ensemble de test: {accuracy_score(Y_test, y_pred2)}")
print(f"Classification Report pour {adaboost_model.__class__.__name__} sur l'ensemble de test:\n{classification_report(Y_test, y_pred2)}")
print("\n")

################# ANALYSE DU MODELE #################################################

plot_permutation_importance(adaboost_model, X_test, Y_test, X_all.columns)

afficher_importance_features(adaboost_model, X_all)

plot_confusion(Y_test, y_pred1)

plot_confusion(Y_test, y_pred2)

plot_results(adaboost_model, Y_test, y_pred1, y_pred2, cv_scores1, cv_scores2)
...
# Afficher les résultats
print(f"Accuracy Score pour {adaboost_model.__class__.__name__} sur l'ensemble de test nevada: {accuracy_score(y_test_ne, y_pred_ne)}")
print(f"Classification Report pour {adaboost_model.__class__.__name__} sur l'ensemble de test nevada :\n{classification_report(y_test_ne, y_pred_ne)}")
print("\n")
...
# Afficher les résultats
print(f"Accuracy Score pour {adaboost_model.__class__.__name__} sur l'ensemble de test colorado: {accuracy_score(y_test_co, y_pred_co)}")
print(f"Classification Report pour {adaboost_model.__class__.__name__} sur l'ensemble de test colorado :\n{classification_report(y_test_co, y_pred_co)}")
print("\n")
######################################################################################################################
...
# Séparer les fonctionnalités (X) de la variable cible (Y)
...
# only use the first N samples to limit training time num_samples = int(len(X_all)*0.1)
num_samples = int(len(X_all2)*0.04)
...
grid_search2 = GridSearchCV(AdaBoostClassifier() , param_grid_svm, cv=5, scoring='accuracy')
...
grid_search2 = GridSearchCV(adaboost_model2 , param_grid_svm , cv=5, scoring='accuracy')
grid_search2.fit(X_train2, Y_train2)
